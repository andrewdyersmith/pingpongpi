    ...
    while not self.terminated:
      ...
      if self.event.wait(1):
        try:
          ...
          cropped_img = img.crop((65,0,215,200)).resize((10,15))
...
class CameraPlayer:
  def __init__(self, time_per_frame):
    ...
    self.camera_processor = ProcessOutput()
    self.running=False
  ...
  def run(self):
    ...
    with picamera.PiCamera(resolution=(320,240)) as camera:
      ...
      time.sleep(2)
      camera.start_recording(self.camera_processor, format='mjpeg')
      while (not self.camera_processor.done) and self.running:
        camera.wait_recording(1.0/15)
      camera.stop_recording()

  def update(self, screen, time):
    if self.camera_processor.next_image:
      current_image = self.camera_processor.next_image
      self.camera_processor.next_image = None

      for y in range(0, min(current_image.size[1], screen.height)):
        for x in range(0, min(current_image.size[0], screen.width)):
          pix = current_image.getpixel((9-x,y))
